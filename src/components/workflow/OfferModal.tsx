...
export function OfferModal({ isOpen, onClose, partnership, onSuccess }: OfferModalProps) {
  ...
  return (
    <>
      <Modal isOpen={step === 'form'} onClose={onClose} title="Draft & Send Campaign Offer" size="2xl">
        <form onSubmit={handleContinueToPreview} className="space-y-8 font-outfit">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {/* Left Column: Compensation */}
            <div className="space-y-5">
              <h3 className="text-base font-semibold text-gray-900 border-b border-gray-100 pb-3">Compensation Details</h3>
              ...
              <div className="space-y-3 pt-2">
                ...
                <label className={`flex items-center justify-between p-4 border rounded-xl cursor-pointer transition-all ${offerForm.shipment_included ? 'border-primary-300 bg-primary-50/30 shadow-sm' : 'border-gray-200 bg-white hover:border-gray-300'}`}>
                  <div>
                    <div className="text-sm font-semibold text-gray-900">Include Product Shipment</div>
                    <div className="text-[11px] text-gray-500 mt-0.5">Creator must provide shipping address</div>
                  </div>
                  <div className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${offerForm.shipment_included ? 'bg-primary-600' : 'bg-gray-200'}`}>
                    <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${offerForm.shipment_included ? 'translate-x-6' : 'translate-x-1'}`} />
                  </div>
                  <input type="checkbox" checked={offerForm.shipment_included} onChange={e => setOfferForm(prev => ({ ...prev, shipment_included: e.target.checked }))} className="sr-only" />
                </label>

                {/* Custom Toggle Switch for Affiliate */}
                <label className={`flex items-center justify-between p-4 border rounded-xl cursor-pointer transition-all ${offerForm.affiliate_enabled ? 'border-primary-300 bg-primary-50/30 shadow-sm' : 'border-gray-200 bg-white hover:border-gray-300'}`}>
                  <div>
                    <div className="text-sm font-semibold text-gray-900">Enable Affiliate Tracking</div>
                    <div className="text-[11px] text-gray-500 mt-0.5">Assign promo codes to track sales</div>
                  </div>
                  <div className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${offerForm.affiliate_enabled ? 'bg-primary-600' : 'bg-gray-200'}`}>
                    <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${offerForm.affiliate_enabled ? 'translate-x-6' : 'translate-x-1'}`} />
                  </div>
                  <input type="checkbox" checked={offerForm.affiliate_enabled} onChange={e => setOfferForm(prev => ({ ...prev, affiliate_enabled: e.target.checked }))} className="sr-only" />
                </label>
            ...
            <div className="space-y-5">
              <div className="flex items-center justify-between border-b border-gray-100 pb-3">
                ...
                <Button type="button" size="sm" variant="outline" className="text-[10px] h-8 px-3 hover:bg-gray-50" onClick={addDeliverable}>
              ...
              {deliverables.length === 0 ? (
                <div className="text-center py-12 bg-gray-50/50 rounded-2xl border-2 border-dashed border-gray-200">
                  <div className="text-sm font-medium text-gray-500 mb-3">No deliverables added yet.</div>
                ...
                <div className="space-y-4 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
                  {deliverables.map((d, i) => (
                    <div key={d.id} className="p-5 border border-gray-200 bg-white rounded-2xl shadow-sm relative group hover:border-gray-300 transition-all">
                      <div className="absolute top-4 right-4 flex items-center justify-between">
                        <span className="text-xs font-bold text-gray-300 uppercase tracking-widest bg-gray-50 px-2 py-1 rounded-md">#{i + 1}</span>
                      </div>
                      <button 
                        type="button" 
                      ...
                      <div className="space-y-4 pr-10">
                        <div className="grid grid-cols-2 gap-4">
                          ...
                          <div className="space-y-1.5">
                            <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block">Type</label>
                            <select
          ...
          <div className="flex justify-end gap-3 pt-6 border-t border-gray-100">
            <Button type="button" variant="outline" className="font-normal text-xs uppercase tracking-widest px-8" onClick={onClose}>
              Cancel
            </Button>
            <Button type="submit" className="bg-primary-600 hover:bg-primary-700 text-white font-normal text-xs uppercase tracking-widest px-8 shadow-md hover:shadow-lg transition-all">
