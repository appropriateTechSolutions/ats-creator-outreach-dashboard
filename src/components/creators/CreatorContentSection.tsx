      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white rounded-xl p-4 border border-gray-100 shadow-sm">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-0.5">
            Track deliverables, reviews, and published metrics.
          </p>
        </div>
        <div className="flex gap-2 w-full sm:w-auto">
          {creatorContent.length > 0 && (
            <Button
              size="sm"
              onClick={handleOpenEmailRequirements}
              className="bg-amber-600 hover:bg-amber-700 text-white font-normal uppercase tracking-widest text-[9px] h-9 px-3"
            >
          ...
          {partnerships.length > 0 ? (
            <Button
              ...
              className="bg-primary-600 hover:bg-primary-700 text-white font-normal uppercase tracking-widest text-[9px] h-9 px-3"
            >
              + Add Deliverable
            </Button>
          ) : (
            <span className="text-[10px] text-gray-400 uppercase italic">
              Add to a campaign first
            </span>
          )}
        </div>
      </div>
      ...
      {creatorContent.length === 0 ? (
        <div className="p-8 text-center bg-gray-50/50 rounded-xl border border-gray-100">
          ...
          <p className="text-xs text-gray-400 mt-1">
            Content requirements will appear here once created.
          </p>
        </div>
      ) : (
              ...
              <tbody className="divide-y divide-gray-50">
                {creatorContent.map((c) => (
                  <tr
                    key={c.id}
                    onClick={() =>
                      navigate(`/content/${c.id}`, { state: { fromCreatorId: creatorId } })
                    }
                  ...
                  >
                    ...
                    <td className="px-4 py-4 align-middle">
                      <div className="text-xs font-medium text-gray-800 flex items-center gap-1.5 capitalize">
                        {c.platform} <span className="text-gray-300">•</span>{' '}
                      ...
                      {c.status === 'published' ? (
                        <div className="text-[10px] font-mono text-gray-600 flex justify-end items-center gap-3">
                          <span className="flex items-center gap-1" title="Views">
                            {c.views || 0} <Eye size={12} />
                          </span>
                          <span className="flex items-center gap-1" title="Likes">
                            {c.likes || 0} <Heart size={12} />
                          </span>
                          <span className="flex items-center gap-1" title="Comments">
                            {c.comments || 0} <MessageCircle size={12} />
                          </span>
                        </div>
                      ) : (
                        <span className="text-[10px] text-gray-400 uppercase italic">
                      ...
                      <div className="flex items-center justify-center gap-2">
                        {c.status === 'submitted' && (
                          <>
                            ...
                            <Button
                              ...
                              onClick={() => handleContentAction(c.id, 'revise')}
                            >
                              Revise
                            </Button>
                          </>
                        ...
                        {c.status === 'published' && (
                          <Button
                            ...
                            onClick={() => handleContentAction(c.id, 'sync')}
                          >
                            Sync
                          </Button>
                        )}
