...
export default function Creators() {
  ...
  const [openMobileDropdown, setOpenMobileDropdown] = useState<'followers' | 'engagement' | 'sort' | null>(null);
                ...
                return (
                  <div key={c.id} className="p-5 active:bg-gray-50 transition-all space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-y-3 gap-x-4 pt-1 text-xs">
                      <div>
                        <span className="text-[10px] font-normal text-gray-400 uppercase tracking-widest block mb-0.5">Category</span>
                        <div className="flex gap-1 flex-wrap animate-none">
                          {c.category?.split(',').slice(0, 2).map((cat, i) => (
                             <span key={i} className="px-1.5 py-0.5 bg-gray-100 text-gray-600 rounded text-[9px] font-normal uppercase tracking-wider">{cat.trim().substring(0,12)}</span>
                          ))}
                        </div>
                      </div>
                      <div>
                      ...
                      <div className="flex gap-4">
                        <div>
                          <span className="text-[10px] font-normal text-gray-400 uppercase tracking-widest block mb-0.5">Relevance</span>
                          <ScoreBadge score={c.relevance_score || 0} />
                        </div>
                        <div>
                          <span className="text-[10px] font-normal text-gray-400 uppercase tracking-widest block mb-0.5">Readiness</span>
                          <ScoreBadge score={c.outreach_readiness_score || 0} />
                        </div>
                      </div>
                    </div>
                 ...
                 >
                   <span>
                     {sortBy === 'followers_desc' ? 'Followers (High→Low)' :
                      ...
                      sortBy === 'engagement_asc' ? 'Engagement (Low→High)' :
                      sortBy === 'relevance_desc' ? 'Relevance (High→Low)' :
                      sortBy === 'readiness_desc' ? 'Readiness (High→Low)' : 'Followers (High→Low)'}
                   </span>
                   <ChevronDown size={16} className="text-gray-400" />
                 </button>
                 {openMobileDropdown === 'sort' && (
                   <div className="absolute left-0 mt-1 w-full bg-white border border-gray-150 rounded-xl shadow-xl z-30 py-1.5 max-h-60 overflow-y-auto">
                     {[
                       ...
                       { val: 'relevance_desc', label: 'Relevance (High→Low)' },
                       { val: 'readiness_desc', label: 'Readiness (High→Low)' }
                     ].map(opt => (
                       <button
                         type="button"
                         key={opt.val}
