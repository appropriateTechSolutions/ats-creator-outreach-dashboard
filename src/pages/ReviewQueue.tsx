    ...
    <div className="space-y-6 max-w-4xl mx-auto pb-12 animate-[fadeIn_0.3s_ease]">
      ...
      <Card className="shadow-xl shadow-gray-200/50 border-gray-200 overflow-hidden">
        <div className="bg-gradient-to-r from-gray-50 to-white px-8 py-6 border-b border-gray-100 flex items-start justify-between">
          <div className="flex items-center gap-5">
            <div className="w-16 h-16 rounded-full bg-primary-600 text-white flex items-center justify-center font-bold text-2xl uppercase shadow-inner">
            ...
            <div>
              ...
              <p className="text-gray-500 font-medium">{currentLead.full_name || 'No full name provided'}</p>
            </div>
          </div>
        ...
        <CardContent className="px-8 py-6 space-y-6">
          <div className="grid grid-cols-2 gap-6">
            <div>
               <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">Audience & Reach</h3>
               <div className="bg-gray-50 rounded-xl p-4 border border-gray-100 space-y-3">
                 <div className="flex justify-between items-center">
                   ...
                   <span className="font-semibold text-gray-900">{currentLead.followers_count?.toLocaleString() || '-'}</span>
                 </div>
                 <div className="flex justify-between items-center">
                   <span className="text-sm text-gray-600">Engagement</span>
                   <span className="font-semibold text-gray-900">{currentLead.engagement_rate ? `${(currentLead.engagement_rate * 100).toFixed(1)}%` : '-'}</span>
                 </div>
                 <div className="flex justify-between items-center">
                   <span className="text-sm text-gray-600">Category</span>
                   <span className="font-semibold text-gray-900 capitalize text-right max-w-[120px] truncate">{currentLead.category || '-'}</span>
                 </div>
                 <div className="flex justify-between items-center">
                   <span className="text-sm text-gray-600">City</span>
                   <span className="font-semibold text-gray-900 capitalize">{currentLead.city || '-'}</span>
                 </div>
               </div>
            </div>

            <div>
               <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">Contact Discovery</h3>
               <div className="bg-gray-50 rounded-xl p-4 border border-gray-100 space-y-3">
                 <div className="flex justify-between items-center">
                   <span className="text-sm text-gray-600 flex items-center gap-2"><Mail size={14} className="text-primary-600"/> Email Present</span>
                   {currentLead.email ? (
                     <span className="px-2 py-0.5 rounded text-xs font-bold bg-success-100 text-success-700">Yes</span>
                   ) : (
                     <span className="px-2 py-0.5 rounded text-xs font-bold bg-error-100 text-error-700">Missing</span>
                   )}
                 </div>
                 {currentLead.email && (
          ...
          {currentLead.research_summary && (
            <div>
              <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">AI Evaluation Summary</h3>
        ...
        <div className="grid grid-cols-2">
          <button 
            ...
            className="flex items-center justify-center gap-2 py-6 bg-red-50 text-red-600 font-bold hover:bg-red-100 transition-colors border-t border-r border-red-100 group"
          >
            <X size={20} className="group-hover:scale-125 transition-transform" /> Reject Lead
          </button>
          <button 
            onClick={() => handleReview('approve')}
            className="flex items-center justify-center gap-2 py-6 bg-success-50 text-success-700 font-bold hover:bg-success-100 transition-colors border-t border-success-100 group"
          >
      ...
      {queue.length > 1 && (
        <div className="pt-6 border-t border-gray-200">
          <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-3">Up Next</p>
          <div className="flex gap-4 overflow-x-auto pb-4">
            {queue.slice(currentIndex + 1, currentIndex + 6).map((lead, idx) => (
              <div key={lead.id} className={`flex-shrink-0 flex items-center gap-2 bg-white px-3 py-2 rounded-lg border border-gray-200 shadow-sm opacity-${100 - (idx * 15)}`}>
                ...
                <div className="text-xs font-semibold text-gray-700">@{lead.handle}</div>
              </div>
            ))}
          </div>
        </div>
