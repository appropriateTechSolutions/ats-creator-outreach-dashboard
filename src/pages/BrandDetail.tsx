...
export default function BrandDetail() {
  ...
  const { data: brand, isLoading: loading, refetch: fetchBrandData } = useBrand(id);
  ...
  // Keep the edit form in sync with the loaded brand.
  useEffect(() => {
    if (brand) setEditFormData(brand);
  }, [brand]);
  ...
  const handleUpdateStatus = async (newStatus: string) => {
    ...
    if (brand) {
      queryClient.setQueryData(queryKeys.brands.detail(id), { ...brand, status: newStatus });
    }
    ...
    try {
      ...
      logger.error(err);
      toast.error(`Failed to update brand status to ${newStatus}`);
  ...
  const canDeleteBrand =
    hasRole(currentUser?.role, ROLE_GROUPS.ADMINS) ||
  ...
  if (!brand) {
    return (
      <div className="p-20 text-center">
        ...
        <Link to="/brands" className="text-primary-600 font-normal mt-4 inline-block">
          Back to Brands
        </Link>
      </div>
    );
    ...
    <div className="space-y-8 max-w-7xl mx-auto pb-20 animate-[fadeIn_0.2s_ease] px-4 sm:px-0">
        ...
        <div className="space-y-4">
          ...
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-3xl sm:text-4xl font-normal text-gray-900 font-outfit uppercase tracking-tight leading-tight">
                {brand.name}
              </h1>
            </div>
              ...
              <div className="flex items-center gap-3">
                ...
                {hasRole(currentUser?.role, ROLE_GROUPS.ADMINS) && (
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() =>
                        handleUpdateStatus(brand.status === 'active' ? 'inactive' : 'active')
                      }
                    ...
                    >
                      <span
                        className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${brand.status === 'active' ? 'translate-x-5' : 'translate-x-0'}`}
                      />
                    </button>
                    <span
                      className={`text-[10px] font-bold uppercase tracking-widest ${brand.status === 'active' ? 'text-primary-600' : 'text-gray-400'}`}
                    >
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <tbody className="divide-y divide-gray-50">
              <tr>
                <td className="px-6 sm:px-8 py-5 w-40 sm:w-64 bg-gray-50/30 text-[10px] font-normal text-gray-400 uppercase tracking-widest border-r border-gray-50 font-outfit">
                  Product Category
                </td>
                <td className="px-6 sm:px-8 py-5 text-sm font-normal text-gray-900">
                  {brand.product_category || 'N/A'}
                </td>
              </tr>
              <tr>
                <td className="px-6 sm:px-8 py-5 bg-gray-50/30 text-[10px] font-normal text-gray-400 uppercase tracking-widest border-r border-gray-50 font-outfit">
                  Official Website
                </td>
                <td className="px-6 sm:px-8 py-5">
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="text-primary-600 font-normal text-sm flex items-center gap-2 hover:underline truncate max-w-[200px] sm:max-w-none inline-flex"
                  >
                    <Globe size={14} className="shrink-0" />{' '}
                    <span className="truncate">{brand.website || 'No URL Provided'}</span>
                  </a>
                </td>
              </tr>
              <tr>
                ...
                <td className="px-6 sm:px-8 py-5 text-sm font-normal text-gray-600 leading-relaxed">
              ...
              <tr>
                ...
                <td className="px-6 sm:px-8 py-5 text-sm font-normal text-gray-600">
      ...
      <Card className="border-none shadow-2xl overflow-hidden bg-white">
        ...
        <table className="w-full text-left">
          <tbody className="divide-y divide-gray-50">
            <tr>
              <td className="px-8 py-5 w-64 bg-gray-50/30 text-[10px] font-normal text-gray-400 uppercase tracking-widest border-r border-gray-50 font-outfit">
                Brand Voice
              </td>
              <td className="px-8 py-5 text-sm font-normal text-gray-900">
                {brand.brand_voice || 'N/A'}
              </td>
            </tr>
            <tr>
              <td className="px-8 py-5 bg-gray-50/30 text-[10px] font-normal text-gray-400 uppercase tracking-widest border-r border-gray-50 font-outfit">
                Content Restrictions
              </td>
              <td className="px-8 py-5 text-sm font-normal text-gray-600 italic">
                {brand.restrictions || 'None specified.'}
              </td>
            </tr>
          </tbody>
        </table>
        ...
        <table className="w-full text-left">
          <tbody className="divide-y divide-gray-50">
            <tr>
              ...
              <td className="px-8 py-5 text-sm font-normal text-gray-900">
                {brand.default_commission_percent}% Default
              </td>
            </tr>
            <tr>
              <td className="px-8 py-5 bg-gray-50/30 text-[10px] font-normal text-gray-400 uppercase tracking-widest border-r border-gray-50 font-outfit">
              ...
              <td className="px-8 py-5 text-sm font-normal text-gray-600 leading-relaxed whitespace-pre-line">
                {brand.affiliate_terms || 'Standard terms apply.'}
              </td>
            </tr>
            <tr>
              <td className="px-8 py-5 bg-gray-50/30 text-[10px] font-normal text-gray-400 uppercase tracking-widest border-r border-gray-50 font-outfit">
              ...
              <td className="px-8 py-5 text-sm font-normal text-gray-600 italic">
                {brand.product_offer_notes || 'No specific offer details.'}
              </td>
            </tr>
          </tbody>
        </table>
        ...
        <table className="w-full text-left">
          <tbody className="divide-y divide-gray-50">
            <tr>
              ...
              <td className="px-8 py-5 text-sm font-normal text-gray-600 italic">
                {brand.notes || 'No internal documentation available.'}
              </td>
            </tr>
          </tbody>
        </table>
            ...
            <tbody className="divide-y divide-gray-50">
              {brand.campaigns && brand.campaigns.length > 0 ? (
                brand.campaigns.map((c) => (
                  <tr
                    ...
                    onClick={() =>
                      navigate(`/campaigns/${c.id}`, {
                        state: { fromBrandId: brand.id, fromBrandName: brand.name },
                      })
                    }
                  ...
                  >
                    ...
                    <td className="px-8 py-5 text-center">
                      <div className="flex gap-1 justify-center flex-wrap">
                        {[
                          ...
                          <span
                            key={`${cat}-${index}`}
                            className="px-2 py-0.5 rounded text-[9px] font-normal uppercase tracking-widest bg-gray-100 text-gray-600 break-keep capitalize border border-gray-200"
                          >
                            {cat}
                          </span>
                        ))}
                    ...
                    <td className="px-8 py-5 text-right text-gray-500 text-[10px] uppercase tracking-widest">
                      {c.created_at || (c as any).createdAt
                        ? format(new Date(c.created_at || (c as any).createdAt), 'MMM d, yyyy')
                        : 'Recently'}
                    </td>
                  </tr>
                ))
                ...
                <tr>
                  <td
                    colSpan={5}
                    className="px-8 py-16 text-center text-gray-400 italic text-sm font-light"
                  >
            ...
            <div className="p-8 border-b border-gray-100 flex justify-between items-center bg-white shrink-0">
              <div>
                ...
                <p className="text-slate-400 text-[10px] font-light mt-1 uppercase tracking-widest font-outfit leading-relaxed">
                  Update creative parameters and commercial terms.
                </p>
              </div>
              <button
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-2 gap-8 items-start">
                    <Input
                      ...
                      onChange={(e) =>
                        setEditFormData({ ...editFormData, product_category: e.target.value })
                      }
                      ...
                      <div className="relative">
                        <Input
                          ...
                          onChange={(e) =>
                            setEditFormData({
                              ...editFormData,
                              default_commission_percent: e.target.value,
                            })
                          }
                          placeholder="10"
                        />
                  ...
                  <div className="grid grid-cols-2 gap-8 items-start">
                    <div className="space-y-1.5">
                      ...
                      <textarea
                        ...
                        onChange={(e) =>
                          setEditFormData({ ...editFormData, brand_description: e.target.value })
                        }
                    ...
                    <div className="space-y-1.5">
                      ...
                      <textarea
                        ...
                        onChange={(e) =>
                          setEditFormData({ ...editFormData, target_audience: e.target.value })
                        }
                        className="w-full bg-white/50 backdrop-blur-sm border border-slate-200 rounded-xl p-4 text-sm font-normal text-slate-900 h-24 focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 outline-none transition-all font-outfit placeholder:text-slate-400"
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-2 gap-8 items-start">
                    <Input
                      ...
                      onChange={(e) =>
                        setEditFormData({ ...editFormData, brand_voice: e.target.value })
                      }
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setEditFormData({ ...editFormData, restrictions: e.target.value })
                      }
                  ...
                  <div className="grid grid-cols-2 gap-8 items-start">
                    <div className="space-y-1.5">
                      ...
                      <textarea
                        ...
                        onChange={(e) =>
                          setEditFormData({ ...editFormData, affiliate_terms: e.target.value })
                        }
                        className="w-full bg-white/50 backdrop-blur-sm border border-slate-200 rounded-xl p-4 text-sm font-normal text-slate-900 h-24 focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 outline-none transition-all font-outfit placeholder:text-slate-400"
                    ...
                    <div className="space-y-1.5">
                      ...
                      <textarea
                        ...
                        onChange={(e) =>
                          setEditFormData({ ...editFormData, product_offer_notes: e.target.value })
                        }
                        className="w-full bg-white/50 backdrop-blur-sm border border-slate-200 rounded-xl p-4 text-sm font-normal text-slate-900 h-24 focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 outline-none transition-all font-outfit placeholder:text-slate-400"
          ...
          <Card className="w-full max-w-sm border-none shadow-3xl bg-white rounded-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-6 text-center">
              ...
              <p className="text-sm font-normal text-gray-500 mb-6 font-outfit">
                ...
                <strong className="text-gray-900">{brand.name}</strong>?
              </p>
